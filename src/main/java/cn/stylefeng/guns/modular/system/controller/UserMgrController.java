...
import cn.stylefeng.guns.config.KeyWordsConfig;
...
public class UserMgrController extends BaseController {
    ...
    public ResponseData edit(@Valid UserDto user, BindingResult result) throws NoPermissionException {
        ...
        if (ShiroKit.hasRole(Const.ADMIN_NAME)) {
            ...
            // 判断关键字
            if(user.getName().contains(KeyWordsConfig.words_kakacl)) {
                throw new ServiceException(StoreExceptionEnum.PARAM_ERROR_80001);
            }
    ...
    @ResponseBody
    public GlobalResponse clerkList(@RequestParam(required = false) String name, @RequestParam(required = false) String beginTime, @RequestParam(required = false) String endTime, @RequestParam(required = false) Integer deptid) {
        GlobalResponse globalresponse = GlobalResponse.getInstanseGlobalResponse();
        Account accountentity = new Account();
        Map<String, Object> maps = new HashMap<>();
        List result = new ArrayList();
        if (ShiroKit.isAdmin() || ShiroKit.getUser().getRoleList().contains(GlobalNumber.INT_ROLE_10)) {
            List<Map<String, Object>> users = userService.selectUsers(null, name, beginTime, endTime, deptid);
            for(int i = GlobalNumber.INT_FIX_0; i < users.size(); i ++) {
                Long id = Long.valueOf(users.get(i).get("id").toString());
                String username = users.get(i).get("name").toString();
                String account = users.get(i).get("account").toString();
                Object roleids = users.get(i).get("roleid");
                accountentity = new Account();
                accountentity.setId(id);
                accountentity.setNickname(username);
                if(roleids == null) {
                    result.add(accountentity);
                } else if(roleids != null && !roleids.toString().contains(GlobalNumber.INT_CLERK_ROLE_7 + "")
                && !roleids.toString().contains(GlobalNumber.INT_STORE_MANAGER_6 + "")) {
                    result.add(accountentity);
                }
            }
        } else {
     ...
     * @return
     */
    @RequestMapping("/storeManagerList")
    @ResponseBody
    public GlobalResponse storeManagerList(@RequestParam(required = false) String name, @RequestParam(required = false) String beginTime, @RequestParam(required = false) String endTime, @RequestParam(required = false) Integer deptid) {
        GlobalResponse globalresponse = GlobalResponse.getInstanseGlobalResponse();
        Account accountentity = new Account();
        Map<String, Object> maps = new HashMap<>();
        List result = new ArrayList();
        if (ShiroKit.isAdmin() || ShiroKit.getUser().getRoleList().contains(GlobalNumber.INT_ROLE_10)) {
            ...
            for(int i = GlobalNumber.INT_FIX_0; i < users.size(); i ++) {
                ...
                String username = users.get(i).get("name").toString();
                String account = users.get(i).get("account").toString();
                String roleids = users.get(i).get("roleid") + "";
                accountentity = new Account();
                accountentity.setId(id);
                accountentity.setNickname(username);
                if(roleids != null && roleids.contains(GlobalNumber.INT_STORE_MANAGER_6 + "")) {
                    result.add(accountentity);
                }
            }
        } else {
     ...
     * @return
     */
    @RequestMapping("/storeManagerJoinList")
    @ResponseBody
    public GlobalResponse storeManagerJoinList(@RequestParam(required = false) String name, @RequestParam(required = false) String beginTime, @RequestParam(required = false) String endTime, @RequestParam(required = false) Integer deptid) {
        GlobalResponse globalresponse = GlobalResponse.getInstanseGlobalResponse();
        Account accountentity = new Account();
        Map<String, Object> maps = new HashMap<>();
        List result = new ArrayList();
